...
class BottomSheet extends React.Component<Props, State> {
    ...
    public componentDidMount(): void {
        ...
        this.headerRef.current!.addEventListener(
            "touchstart",
    ...
    public componentWillUnmount(): void {
        ...
        this.headerRef.current!.removeEventListener(
            "touchstart",
            this.handleTouchStart
        );
        this.headerRef.current!.removeEventListener(
    ...
    private handleTouchEnd = (e: TouchEvent) => {
        if (!this.state.isMaximized) {
            if (
                ...
                -this.state.touchDiffY! > this.maximizeThreshold
            ) {
                this.setState({ touchEndY: 20, isMaximized: true });
            } else if (
                this.state.touchDiffY! >
                this.state.initialMainHeight * 0.382
            ) {
                this.setState(
                    { touchEndY: window.visualViewport!.height },
                    () => {
                        setTimeout(
                            () => this.handleClickBackground({} as MouseEvent),
