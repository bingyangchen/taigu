...
class Home extends React.Component<Props, State> {
    ...
    public componentDidUpdate(
        ...
        if (
            prevProps.tseIndexRealtimePrices !==
                this.props.tseIndexRealtimePrices ||
            prevProps.otcIndexRealtimePrices !==
                this.props.otcIndexRealtimePrices
        ) {
            this.updateIndexLineCharts();
        }
        if (
                ...
                <div className={styles.left}>
                    <div className={styles.upper}>
                        <div className={styles.market_index}>
                            ...
                            <div
                                className={`${
                                    styles.price
                                } ${this.getIndexPriceExtraClass("tse")}`}
                            >
                                ...
                                <div className={styles.fluct_price}>
                                    {this.getIndexFluctPriceText(
                                        this.latestTsePriceInfo?.fluct_price ??
                                            0
                                    )}{" "}
                                    {this.getIndexFluctPercentText(
                                        this.latestTseFluctPercent
                                    )}
                                </div>
                            </div>
                        </div>
                        <div className={styles.market_index}>
                            ...
                            <div
                                className={`${
                                    styles.price
                                } ${this.getIndexPriceExtraClass("otc")}`}
                            >
                                <div className={styles.price}>
                                ...
                                <div className={styles.fluct_price}>
                                    {this.getIndexFluctPriceText(
                                        this.latestOtcPriceInfo?.fluct_price ??
                                            0
                                    )}{" "}
                                    {this.getIndexFluctPercentText(
                                        this.latestOtcFluctPercent
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className={styles.lower}>
                        ...
                        <div className={styles.body}>
                            <div
                                className={styles.cash_invested_chart_container}
                            >
                            ...
                            <div className={styles.controls}>
                                <div className={styles.time_span_options}>
                                    <div
                                        className={
                                            styles.time_span_options_inner
                                        }
                                    >
                                        <span
                                            className={this.getTimeSpanOptionClass(
                                                30
                                            )}
                                            onClick={() =>
                                                this.handleClickTimeSpanOption(
                                                    30
                                                )
                                            }
                                        >
    ...
    private get tseInfoDate(): string {
        return (
            Object.values(this.props.tseIndexRealtimePrices)[0]?.date ??
            "0000-00-00"
        );
    }
    private get otcInfoDate(): string {
        return (
            Object.values(this.props.otcIndexRealtimePrices)[0]?.date ??
            "0000-00-00"
        );
    }
    private get latestTsePriceInfo(): IndexPriceInfo | null {
        const maxNum = Math.max(
            ...Object.keys(this.props.tseIndexRealtimePrices).map((k) =>
                parseInt(k)
            )
        );
    ...
    private get latestOtcPriceInfo(): IndexPriceInfo | null {
        const maxNum = Math.max(
            ...Object.keys(this.props.otcIndexRealtimePrices).map((k) =>
                parseInt(k)
            )
        );
    ...
    private getIndexPriceExtraClass(index: "tse" | "otc"): string {
        if (index === "tse") {
            ...
            const prevTseClosePrice =
                (Object.values(this.props.tseIndexRealtimePrices)[0]?.price ??
                    0) -
                (Object.values(this.props.tseIndexRealtimePrices)[0]
                    ?.fluct_price ?? 0);
            ...
            const prevOtcClosePrice =
                (Object.values(this.props.otcIndexRealtimePrices)[0]?.price ??
                    0) -
                (Object.values(this.props.otcIndexRealtimePrices)[0]
                    ?.fluct_price ?? 0);
        ...
        this.setState((state, props) => {
            ...
            for (let i = 0; i <= 270; i++) {
                if (props.tseIndexRealtimePrices.hasOwnProperty(i.toString())) {
                    tseChartData.push([
                        i,
                        props.tseIndexRealtimePrices[i].price,
                    ]);
            ...
            for (let i = 0; i <= 270; i++) {
                if (props.otcIndexRealtimePrices.hasOwnProperty(i.toString())) {
                    otcChartData.push([
                        i,
                        props.otcIndexRealtimePrices[i].price,
                    ]);
