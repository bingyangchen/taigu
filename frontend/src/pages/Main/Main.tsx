...
class Main extends React.Component<Props, State> {
    ...
    public componentDidUpdate(
        ...
        if (prevProps.router.location.key !== this.props.router.location.key) {
            if (
                ...
                prevProps.router.location.state ===
    ...
    public render(): React.ReactNode {
        return (
            <main className={styles.main} ref={this.mainRef}>
                ...
                {Util.isMobile && (
                    <Footer subpages={this.subpages.slice(0, 4)} />
                )}
            </main>
        );
    }
    ...
    private handleNonCacheResponse = async (
        ...
        if (e.data.authorized) {
            ...
            if (/account\/me[/]?$/gs.test(e.data.url)) {
                this.props.dispatch(
                    refreshAccountInfoWithNonCacheResponse(data)
                );
                ...
                this.props.dispatch(
                    refreshMarketIndexWithNonCacheResponse(data)
                );
            } else if (/memo\/trade-plans[/]?$/gs.test(e.data.url)) {
                this.props.dispatch(
                    refreshTradePlansWithNonCacheResponse(data)
                );
            } else if (/memo\/company-info[/]?$/gs.test(e.data.url)) {
                this.props.dispatch(
                    refreshAllCompanyInfoWithNonCacheResponse(data)
                );
    ...
    private fetchNecessaryStockInfo = async (): Promise<void> => {
        ...
        const sids = Array.from(
            new Set([...hodlingSids, ...this.props.favorites])
        );
