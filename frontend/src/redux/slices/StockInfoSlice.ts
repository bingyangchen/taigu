...
export const fetchRealtimeMarketIndex = createAsyncThunk(
    ...
    async (): Promise<MarketIndex> => {
        const response = await Api.sendRequest("stock/market-index", "get");
        if (response?.success) return response.data;
        else throw Error("Failed to fetch market index");
    }
);

export const fetchStockInfo = createAsyncThunk(
    "stockInfo/fetchStockInfo",
    async (sids: string[]): Promise<StockInfo[]> => {
...
export const fetchSingleStockHistoricalPrices = createAsyncThunk(
    ...
    async (payload: {
        ...
        if (response?.success) {
            const data: { date: string; price: number }[] = response.data.map(
                (row: { date: string; price: number }) => {
                    return {
                        date: new Date(Date.parse(row.date)).toLocaleDateString(
                            "af"
                        ),
                        price: row.price,
                    };
        ...
        } else throw Error("Failed to fetch historical prices");
    }
);
        ...
        builder
            ...
            .addCase(fetchStockInfo.fulfilled, (state, action) => {
                [...action.payload].forEach((info) => {
                    state.sidStockInfoMap[info.sid] = info;
                });
            })
            ...
            .addCase(
                fetchSingleStockHistoricalPrices.fulfilled,
                (state, action) => {
                    ...
                    if (frequency === "DAILY") {
                        ...
                        data.forEach((row) => {
                            state.sidHistoricalPricesMap[sid].daily![row.date] =
                                row.price;
                        });
...
export const getSidMarketValueMap = (
    ...
    for (const [sid, inventoryCosts] of Object.entries(stockWarehouse)) {
        result[sid] = calculateMarketValue(
            sidStockInfoMap[sid],
            inventoryCosts
        );
    }
