...
export const fetchAllTradeRecords = createAsyncThunk(
    ...
    async (): Promise<Omit<TradeRecordState, "isWaiting">> => {
        ...
        if (response?.success) return await computeNewState(response.data);
...
export const createRecord = createAsyncThunk(
    ...
    async (
        ...
        } else throw Error(response?.error || "Failed to create trade record");
    }
);
...
export const updateRecord = createAsyncThunk(
    ...
    async (
        ...
        if (response?.success) {
            ...
            return await computeNewState(
                rootState.tradeRecord.tradeRecords
                    .map((r) => (r.id === response.data.id ? response.data : r))
                    .sort(
                        (a, b) =>
                            Date.parse(b.deal_time) - Date.parse(a.deal_time)
                    )
            );
        } else throw Error(response?.error || "Failed to update trade record");
    }
);
...
export const deleteRecord = createAsyncThunk(
    ...
    async (
        ...
        if (response?.success) {
            const rootState = thunkAPI.getState() as RootState;
            return await computeNewState(
                [...rootState.tradeRecord.tradeRecords].filter(
                    (r) => r.id !== id
                )
            );
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchAllTradeRecords.fulfilled, (state, action) => {
                ...
                state.cashInvestedChartData =
                    action.payload.cashInvestedChartData;
                state.tradeVolumeChartData =
                    action.payload.tradeVolumeChartData;
